-- =====================================================
-- XENTRA - Integración Completa de Atributos Dinámicos (CORREGIDO)
-- =====================================================
...
-- 3. Eliminar vista existente y recrearla (solución al conflicto)
...
-- 7. Asignación automática de plantillas a categorías existentes (solo si existen plantillas)
DO $$
BEGIN
    -- Tecnología
    IF EXISTS (SELECT 1 FROM public.plantillas_atributos WHERE sector_negocio = 'tecnologia') THEN
    ...
    -- Textil
    IF EXISTS (SELECT 1 FROM public.plantillas_atributos WHERE sector_negocio = 'textil') THEN
        UPDATE public.categorias SET plantilla_atributos_predeterminada = (
            SELECT pa.id FROM public.plantillas_atributos pa 
    ...
    -- Hogar
    IF EXISTS (SELECT 1 FROM public.plantillas_atributos WHERE sector_negocio = 'hogar') THEN
        UPDATE public.categorias SET plantilla_atributos_predeterminada = (
            SELECT pa.id FROM public.plantillas_atributos pa 
...
SELECT 
    ...
    COUNT(DISTINCT c.id) FILTER (WHERE c.plantilla_atributos_predeterminada IS NOT NULL) as categorias_con_plantilla
FROM public.plantillas_atributos pa
LEFT JOIN public.definiciones_atributos da ON da.plantilla_id = pa.id  
LEFT JOIN public.categorias c ON c.plantilla_atributos_predeterminada = pa.id
WHERE pa.empresa_id = '70330390-9e39-4662-8f9e-97e1a2e16342' OR pa.empresa_id IS NULL;